...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        # print('h_cur', h_cur.shape)  #1, 32, 192, 256, 32
        # print('c_cur', c_cur.shape)  #1, 32, 192, 256, 32
        # print('input_tensor', input_tensor.shape)  #1, 32, 192, 256, 32

        combined = torch.cat([input_tensor, h_cur], dim=1)  # concatenate along channel axis
        # print('combined', combined.shape)  #1, 64, 192, 256, 32
        combined_conv = self.conv(combined)
        # print('combined_conv', combined_conv.shape)   #1, 128, 192, 256, 32
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        if not self.batch_first:
            # (t, b, c, h, w) -> (b, t, c, h, w)
            input_tensor = input_tensor.permute(1, 0, 2, 3, 4, 5).to(device)
        # print('input_tensor', input_tensor.shape)  #1, 8, 32, 192, 256, 32
        #取出图片数据,下面初始化使用
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                ...
                # print('output_inner', output_inner[0].shape)
